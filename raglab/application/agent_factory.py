...
“给我今天的日报”“查看某日日报”默认表示读取当前 RAG 索引中
已有的日报，不是直接重新执行更新流水线。
...
“生成今天的日报”“制作今日技术日报”表示用户要求得到一份今天
可用的日报。应先按 today + daily_brief 精确查询，避免已有数据时
重复更新；如果今天的数据不存在，则缺失的是完成任务所需的前置
数据，必须继续执行下面的自动补全流程，不能直接回答“无法生成”。

## 5. 今日日报缺失时的任务闭环

对于“生成 / 制作今天的日报”：

1. 先调用 search_github_intelligence 精确查询 today 的 daily_brief；
2. 如果存在，直接基于检索结果返回，禁止重复更新；
3. 如果不存在，调用 load_skill 加载 github-intelligence-update；
4. 下一步调用 update_github_intelligence；该高风险写操作必须经过
   Tool Policy 与 HITL，不能绕过人工审批；
5. 用户批准并恢复执行后，再查询 today 的 daily_brief，并返回新日报。

加载 Skill 或触发 HITL 不是任务结束。批准更新后必须继续完成原始的
“生成今日日报”目标。

## 6. 其他情况下只有明确写操作意图才允许更新
...
2. 不得自动删除过滤条件后改搜其他日期；
3. 不得拿最近历史日报冒充目标日期日报；
4. 普通“查看 / 查询”请求不得因为失败就自行升级为写操作；
5. 可以提示用户如需重新采集，可明确要求更新或刷新。

上述限制不适用于第 5 节定义的“生成 / 制作今日日报”任务闭环。
""".strip()
...
def build_agent(
    ...
    # --------------------------------------------------------
    #
    # 这条规则区分只读查看与生成任务：已有今日日报直接读取；
    # 明确要求生成且今日数据缺失时，经 HITL 补齐数据后继续生成。
